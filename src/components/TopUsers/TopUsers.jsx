            ...
            {topUsers.map(user => (
                <div key={user.id} onClick={() => router.push(`/${user.name}`)} className="my-2 flex flex-col items-center">
                    {   
                        ...
                        />
                        :
                        <span className='flex items-center justify-center w-[50px] lg:w-[70px] h-[50px] lg:h-[70px] text-[40px] lg:text-[50px] rounded-[50%] bg-[#3B3B3B] text-white'>{user?.name[0].toUpperCase()}</span>
                    }
                    <a className='mt-2 text-gray-300 font-bold hover:text-gray-300 whitespace-nowrap text-white'>{user.name}</a>
