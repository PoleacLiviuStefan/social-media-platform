...
const CheckAge = ({ checkAgeToParent }) => {
  ...
  const handleAgeConfirmation = () => {
    Cookies.set('ageConfirmed', 'true', { expires: 365 });
    window.location.reload(); // Reload to re-check the cookie
  };
 ...
 py-[4rem] bg-gray-800  rounded-[12px] h-[35rem] w-[25rem]'>
        ...
        <button onClick={handleAgeConfirmation} className='mt-4 font-bold bg-[#FF0000] text-[18px]  ease-in-out duration-[.3s] hover:bg-[#e6001a] hover:outline-none hover:border-0'>I AM 18 OR OLDER - ENTER</button>
      </div>
    </div>
  );
}
