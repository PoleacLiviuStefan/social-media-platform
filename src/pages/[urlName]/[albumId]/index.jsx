...
import { FaPlay, FaShare, FaCheck, FaRegBookmark, FaRetweet } from "react-icons/fa";
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        try {
            ...
            const videoMaterial = albumResponse.data.album.content.filter((file) => file.name.endsWith(".mp4")) || [];
            ...
            <div className="flex items-center gap-2 lg:gap-4">
              ...
              >
                { userImage ?
                  ...
                  <img src={ userImage.includes("google") ? userImage:`${SERVER_URL}/uploads/${userImage}`} className="cursor-pointer rounded-[50%] w-[70px] h-[70px]" />
                  :
                  <span className='flex items-center justify-center cursor-pointer rounded-[50%] w-[70px] h-[70px] text-[42px] bg-[#3B3B3B] '>{userName[0].toUpperCase()}</span>
                }
...
>
  ...
  <span className="hidden lg:inline">SHARE</span>
              ...
              {
                user===null &&
              <a onClick={()=>router.push("/login")} className="cursor-pointer font-bold text-white hover:text-white">Login to interact with the post</a>
}
            </div>
          </div>
        </div>
    ...
    <div key={index} className="flex items-center justify-center w-full lg:h-[40rem] lg:bg-black">
      ...
      {!playing[index] && (
        ...
        <div className="flex flex-col">
          ...
          {!commentInput ? (
            user==null ?
            <a onClick={()=>router.push("/login")} className="cursor-pointer font-bold text-white hover:text-white">Login to add a comment</a>
            : 
            <button
        ...
        <div  className="mt-[1rem] grid grid-cols-2 md:grid-cols-4  xl:grid-cols-5 items-center gap-4 lg:gap-6 xl:gap-10 justify-center flex-wrap w-full">
          {sugestedPosts.map((album, index) => {
            const images = album.content.filter(
              (file) =>
